...
1. **Thiết kế schema và DTO**
   - Tạo Mongoose schema `material.schema.ts` tồn tại trong `backend/src/schemas`.
   ...
   - Tạo DTOs: `create-material.dto.ts`, `update-material.dto.ts` trong `backend/src/material/dto`.
   - Sử dụng class-validator để thêm rules (required, enums...).

2. **Repository**
   - Tạo `material.repository.ts` dưới `backend/src/material` với phương thức truy vấn cơ bản: findAll, findOne, create, update, remove.
   - Dùng Mongoose Model injection.

3. **Service**
   - Tạo `material.service.ts` xử lý logic: kiểm tra unique material_code, các business rule.
   - Nhúng repository via DI.
   - Thêm phương thức tương ứng cho CRUD.

4. **Controller**
   - `material.controller.ts` với các route: GET /materials, GET /materials/:id, POST /materials, PATCH /materials/:id, DELETE /materials/:id.
   - Áp dụng validation pipe, JWT/AuthGuard/RoleGuard nếu cần, tham khảo pattern module khác.

5. **Module**
   - Cập nhật `material.module.ts` import services, controllers, schemas.
   - Đăng ký vào `app.module.ts` nếu chưa.
