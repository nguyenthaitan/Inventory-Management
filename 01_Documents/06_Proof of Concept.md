...
1.  **Cài đặt và cấu hình Keycloak** - Cài đặt Keycloak server (có thể dùng Docker hoặc bản cài đặt chính thức).
...
**Hạn chế của giải pháp hiện tại:** PoC chứng minh luồng authentication cơ bản nhưng chưa xử lý các yêu cầu production-grade (HA, key rotation, token revocation, secure token storage).
