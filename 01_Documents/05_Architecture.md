...
### 3. Deployment View - VPN Server + Domain Name Architecture
...
└────────────────────────────┬────────────────────────────────────┘
                    ...
                    │ (inventory.com) │
                    └────────┬────────┘
                             │ Resolve to VPN Server IP
                    ┌────────▼─────────────────┐
                    │   VPN SERVER             │
                    │   (Single Ubuntu/CentOS) │
                    │   IP: xxx.xxx.xxx.xxx    │
                    └────────┬─────────────────┘
...
**1. VPN Server (Ubuntu/CentOS Linux)**
- **Mục đích:** Single point deployment chứa toàn bộ hệ thống
- **Thông số:**
  - CPU: 4+ cores
  - RAM: 8GB+
  - Disk: 100GB+ (cho database + logs)
  - OS: Ubuntu 22.04 LTS / CentOS 8+
  - Docker: Docker Engine 24.0+
  - Docker Compose: 2.20+
- **Network Configuration:**
  - Public IP: Gán domain name qua DNS
  - VPN Access: OpenVPN / WireGuard (tùy chọn, để remote admin)
  - Internal Docker Network: `inventory-net` (172.20.0.0/16)

**2. Frontend (React) - Docker Container**
- **URL:** `https://inventory.yourdomain.com/` (hoặc `https://inventory.yourdomain.com:443`)
- **Port Internal:** 80 / 443 (via nginx reverse proxy)
- **Container Image:** `inventory-frontend:latest`
- **Volumes:** Không cần volume (stateless SPA)
- **Environment Variables:**
  ```
  REACT_APP_API_URL=https://inventory.yourdomain.com/api
  REACT_APP_KEYCLOAK_URL=https://inventory.yourdomain.com/auth
  ```

**3. Backend API (NestJS) - Docker Container**
- **URL:** `https://inventory.yourdomain.com/api` (behind nginx proxy)
- **Port Internal:** 3000 (không expose ra ngoài, chỉ docker network)
- **Container Image:** `inventory-backend:latest`
- **Volumes:**
...
- **Environment Variables:**
  ...
  KEYCLOAK_REALM=inventory-management
  KEYCLOAK_CLIENT_ID=inventory-backend
  KEYCLOAK_CLIENT_SECRET=${KEYCLOAK_CLIENT_SECRET}
  FRONTEND_ORIGIN=https://inventory.yourdomain.com
  ```

**4. Keycloak IdP - Docker Container**
- **URL:** `https://inventory.yourdomain.com/auth` (behind nginx proxy)
- **Port Internal:** 8080 (không expose ra ngoài)
- **Container Image:** `quay.io/keycloak/keycloak:23-alpine`
- **Database:** MongoDB hoặc PostgreSQL (embedded H2 cho dev)
- **Environment Variables:**
  ```
  KC_DB=postgres
  ...
  KC_HOSTNAME=inventory.yourdomain.com
  KC_HOSTNAME_PATH=/auth
  KC_HOSTNAME_STRICT=false
  ```

**5. MongoDB - Docker Container**
- **URL:** `mongodb://mongo:27017` (internal only)
- **Port Internal:** 27017
- **Container Image:** `mongo:7-alpine`
- **Volumes:**
...
- **URL:** `redis://redis:6379` (internal only)
- **Port Internal:** 6379
- **Container Image:** `redis:7-alpine`
- **Volumes:** `redis_data:/data` (persistent)
- **Command:** `redis-server --appendonly yes`

**7. Nginx Reverse Proxy - Docker Container**
- **URL:** Listen HTTPS 443, HTTP 80
- **Port External:** 80, 443 (expose từ docker)
- **Container Image:** `nginx:alpine`
- **Volumes:**
  - `./nginx.conf:/etc/nginx/nginx.conf` (config)
  - `./ssl/cert.pem:/etc/nginx/ssl/cert.pem` (SSL certificate)
  - `./ssl/key.pem:/etc/nginx/ssl/key.pem` (SSL private key)
- **Responsibilities:**
  - HTTP → HTTPS redirect
  - Frontend proxying
  - Backend API proxying (`/api` → backend:3000)
  - Keycloak proxying (`/auth` → keycloak:8080)
  - SSL/TLS termination
  - CORS headers

**8. ELK Stack (Elasticsearch, Logstash, Kibana)**
- **Elasticsearch:** `http://elasticsearch:9200` (internal)
- **Kibana:** `https://inventory.yourdomain.com/kibana` (internal access only)
- **Logstash:** `logstash:5000` (receive logs from backend)
- **Containers:** elasticsearch:8, kibana:8, logstash:8

#### Network Configuration Diagram

```
┌──────────────────────────────────────────────────────────┐
│              VPN Server (Linux Host)                      │
│              IP: 192.168.1.100 (example)                │
├──────────────────────────────────────────────────────────┤
│                                                           │
│  ┌─────────────────────────────────────────────────────┐ │
│  │  Docker Container Network (inventory-net)          │ │
│  │  Bridge: 172.20.0.0/16                             │ │
│  │                                                     │ │
│  │  ┌─────────────┐  ┌──────────────┐  ┌────────────┐ │ │
│  │  │ nginx:80,   │  │ backend:3000 │  │ mongo:27017│ │ │
│  │  │ 443         │  │              │  │            │ │ │
│  │  │             │──│ (internal)   │  │ (internal) │ │ │
│  │  │ (proxy)     │  │              │  │            │ │ │
│  │  └─────────────┘  └──────────────┘  └────────────┘ │ │
│  │        │                                            │ │
│  │        ├─→ frontend:5173 (Vite SPA)                │ │
│  │        ├─→ api routing                             │ │
│  │        └─→ keycloak:8080                           │ │
│  │                                                     │ │
│  │  ┌─────────────┐  ┌──────────────┐                 │ │
│  │  │ redis:6379  │  │ ELK Stack    │                 │ │
│  │  │ (internal)  │  │ (ES:9200)    │                 │ │
│  │  └─────────────┘  └──────────────┘                 │ │
│  │                                                     │ │
│  └─────────────────────────────────────────────────────┘ │
...
└──────────────────────────────────────────────────────────┘
    ...
    └────────────┘
         ▲
    DNS: inventory.yourdomain.com → 192.168.1.100
```
...
sudo certbot certonly --standalone -d inventory.yourdomain.com -d auth.inventory.yourdomain.com

# Certificates lưu tại: /etc/letsencrypt/live/inventory.yourdomain.com/
# - cert.pem (certificate)
# - privkey.pem (private key)

# Mount vào Docker:
volumes:
  - /etc/letsencrypt/live/inventory.yourdomain.com/cert.pem:/etc/nginx/ssl/cert.pem:ro
  - /etc/letsencrypt/live/inventory.yourdomain.com/privkey.pem:/etc/nginx/ssl/key.pem:ro
...
# Copy vào: ./ssl/cert.pem, ./ssl/key.pem
```
...
services:
  ...
  backend:
    ...
    environment:
      NODE_ENV: production
      DATABASE_URL: mongodb://admin:${MONGO_PASSWORD}@mongo:27017/inventory
      REDIS_URL: redis://redis:6379
      JWT_SECRET: ${JWT_SECRET}
      KEYCLOAK_URL: http://keycloak:8080
      FRONTEND_ORIGIN: https://inventory.yourdomain.com
  ...
  # Keycloak
  keycloak:
    image: quay.io/keycloak/keycloak:23-alpine
    container_name: inventory-keycloak
    environment:
      KC_DB: postgres
      KC_DB_URL: jdbc:postgresql://postgres:5432/keycloak
      KC_DB_USERNAME: keycloak
  ...
  # Elasticsearch
  elasticsearch:
    image: docker.elastic.co/elasticsearch/elasticsearch:8-alpine
  ...
  # Kibana
  kibana:
    image: docker.elastic.co/kibana/kibana:8-alpine
...
http {
    ...
    server {
        ...
        # Kibana (internal only - optional, can restrict via IP)
        location /kibana/ {
...
# Clone repository
git clone <repo-url> /opt/inventory-management
...
```bash
# Health check
curl https://inventory.yourdomain.com/api/health

# Check frontend
curl -I https://inventory.yourdomain.com/

# Check Keycloak
curl -I https://inventory.yourdomain.com/auth/
```

#### Tầng Bảo mật (Security - Keycloak)

Backend và Frontend thực hiện xác thực và định danh người dùng thông qua **Keycloak** (IdP) trong Docker network. Mọi truy cập từ internet đều đi qua **HTTPS** via nginx reverse proxy, sử dụng chuẩn **OIDC/OAuth2**, Access Token dạng **JWT** được Backend kiểm tra chữ ký trước khi cho phép truy cập tài nguyên.

#### Tầng Dữ liệu (Data Tier)

Được triển khai trên VPN Server dưới dạng Docker containers:

- **MongoDB:** Lưu trữ dữ liệu chính của hệ thống (persistent volume: `mongo_data`)
- **Redis:** Xử lý Caching và cơ chế **Locking tồn kho** với tốc độ cực nhanh, tránh xung đột dữ liệu (persistent volume: `redis_data`)
- **PostgreSQL:** Database cho Keycloak (persistent volume: `postgres_data`)

#### Tầng Giám sát (Observability Tier)

- **Elasticsearch:** Lưu trữ logs từ Backend
- **Kibana:** Dashboard để visualize logs (internal access via `https://inventory.yourdomain.com/kibana`)
- **Prometheus & Grafana:** (Optional) Có thể thêm để monitor metrics từ VPN Server
