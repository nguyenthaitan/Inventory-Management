...
export class InventoryLotService {
  ...
  async getLotById(lot_id: string): Promise<InventoryLotDocument> {
    const lot = await this.inventoryLotRepository.findById(lot_id);
    if (!lot) throw new NotFoundException(`Inventory lot ${lot_id} not found`);
    return lot;
  }
  ...
  async getLotsByIds(lot_ids: string[]): Promise<InventoryLotDocument[]> {
    return this.inventoryLotRepository.findByLotIds(lot_ids);
  }

  /** Alias for updateStatus — validates transition then returns raw document */
  async updateLotStatus(
    lot_id: string,
    status: string,
  ): Promise<InventoryLotDocument> {
    await this.updateStatus(lot_id, status);
    const lot = await this.inventoryLotRepository.findById(lot_id);
    return lot!;
  }

  /** Alias for update — validates then returns raw document */
  async updateLot(
    lot_id: string,
    data: Partial<InventoryLotDocument>,
  ): Promise<InventoryLotDocument> {
    await this.update(lot_id, data as UpdateInventoryLotDto);
    const lot = await this.inventoryLotRepository.findById(lot_id);
    return lot!;
  }
  ...
  async bulkQuarantine(lot_ids: string[]): Promise<InventoryLotDocument[]> {
    return this.inventoryLotRepository.updateManyStatus(
      lot_ids,
      InventoryLotStatus.QUARANTINE,
    );
  }
