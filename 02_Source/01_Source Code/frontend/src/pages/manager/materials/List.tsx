...
const getColumns = (onEdit: (m: Material) => void, onDelete: (m: Material) => void): ColumnsType<Material> => [
	...
	{ title: 'Ngày tạo', dataIndex: 'created_date', key: 'created_date',
...
const MaterialList: React.FC = () => {
	...
	async function fetchData() {
		...
		try {
			...
			let result: Material[] = Array.isArray(res) ? res : [];
			let filtered: Material[] = result;
			if (search) filtered = filtered.filter(m => m.material_name?.toLowerCase().includes(search.toLowerCase()) || m.material_id?.toLowerCase().includes(search.toLowerCase()));
	...
	return (
		<div>
			...
			<Space style={{ marginBottom: 16 }}>
				...
				<Button type="primary" onClick={() => show()}>Thêm mới</Button>
			</Space>
			<Table
			...
			>
				...
				>
					<Form.Item name="material_id" label="Mã vật tư" rules={[{ required: true, message: 'Vui lòng nhập mã vật tư' }]} validateTrigger="onChange">
						<Input />
					</Form.Item>
					<Form.Item name="part_number" label="Part Number" rules={[{ required: true, message: 'Vui lòng nhập Part Number' }]} validateTrigger="onChange">
						<Input />
					</Form.Item>
					<Form.Item name="material_name" label="Tên vật tư" rules={[{ required: true, message: 'Vui lòng nhập tên vật tư' }]} validateTrigger="onChange">
						<Input />
					</Form.Item>
					<Form.Item name="material_type" label="Loại vật tư" rules={[{ required: true, message: 'Vui lòng chọn loại vật tư' }]} validateTrigger="onChange">
					...
					<Form.Item name="status" label="Trạng thái" rules={[{ required: true, message: 'Vui lòng chọn trạng thái' }]} validateTrigger="onChange">
